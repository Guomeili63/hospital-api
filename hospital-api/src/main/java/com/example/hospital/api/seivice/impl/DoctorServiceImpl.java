package com.example.hospital.api.seivice.impl;
...
import com.example.hospital.api.seivice.DoctorService;
...
//                    .build());
            ...
            client.putObject(PutObjectArgs.builder().bucket("hospital").object("doctor/doctor.jpg")
    ...
    public void insert(Map param) throws HospitalException {
        ...
        //验证是否存在重复记录
        String entity_1Pid = entity_1.getPid();
        long countByPid = doctorDao.searchCountByPid(entity_1Pid);
        if(countByPid>0){
            throw new HospitalException("该医生已存在");
        }
